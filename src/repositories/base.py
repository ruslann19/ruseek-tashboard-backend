...
class BaseRepository(Generic[T]):
    ...
    async def get_all(self, *expressions, **filters) -> list[T]:
        query = select(self.model).where(*expressions).filter_by(**filters)
        result = await self.session.execute(query)
        return list(result.scalars().all())

    async def get_one_or_none(self, *expressions, **filters) -> T | None:
        query = select(self.model).where(*expressions).filter_by(**filters)
        result = await self.session.execute(query)
